...
bool Problem::Solve(int iterations) {
    ...
    // save lambda
    ofstream lambda;
    lambda.open("lambda.csv",ios::out);
...
//            alpha_ = alpha / ((tempChi - currentChi_) / 2 + 2*alpha);
            ...
            // 更新状态量  X = X+ delta_x / X = X + alpha*X
            UpdateStates();

            // 判断当前步是否可行以及 LM 的 lambda 怎么更新
#ifdef USE_LM_2
            oneStepSuccess = IsGoodStepInLM2();
#elif defined(USE_LM_3)
            oneStepSuccess = IsGoodStepInLM3();
#else
...
//                stop = (b_max <= 1e-12);
    ...
    lambda.close();
    //CHECK(lambda) << "Writing lambda to csv file failed!";
    std::cout << "Writing lambda to csv file succeed!" << std::endl;
...
// 参考The Levenvberg-Marquardt algorithm fot nonlinear squares curve-fitting problems
// section 4
